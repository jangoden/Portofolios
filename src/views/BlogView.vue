        ...
        <div class="flex flex-col text-left">
          ...
          <div class="hidden md:block">
            ...
            <div class="mt-3 flex flex-wrap gap-1">
              <span class="py-2 px-3 rounded-2xl bg-[#1e1e1f] hover:bg-white/20 text-white text-xs cursor-pointer">Tulisan</span>
              <span class="py-2 px-3 rounded-2xl bg-[#1e1e1f] hover:bg-white/20 text-white text-xs cursor-pointer">Carita</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getArticles() {
      try {
        const response = await axios.get('https://domainmu.com/api/posts'); // Ganti URL ini dengan endpoint API Filament-mu
